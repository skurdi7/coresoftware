...
int CMDistortionAnalysisPhiRFull(int nMaxEvents = -1) {
  ...
  for (int ifile=0;ifile < nEvents;ifile++){
    ...
    TH2F *hCompareRTruevFluctNeg = new TH2F("hCompareRTruevFluct", "Compare True R Distortion Fluctuation and True Charge Fluctuation, Phi,R binning, Negative Side (R > 30); fluct charge (#mum); true shift (#mum)",nbins,-1e6,1e6,nbins,-1e6,1e6);
    TH2F *hCompareRDiffvFluctNeg = new TH2F("hCompareRDiffvFluct", "Compare Difference between R Model and True R vs True Charge Fluctuation, Phi,R binning, Negative Side (R > 30); fluct charge (#mum); shift difference (#mum)",nbins,-1e6,1e6,nbins,-1e6,1e6);

...
int CMCompareRvsIntFluct(int nMaxEvents = -1) {
  ...
  for (int ifile=0;ifile < nEvents;ifile++){
    ...
    for(int ihist=0;ihist < 2;ihist++){//should be ihist < 10 to run over all
      ...
      hCompareRDiffvIntFluctSmallRPosRegion[5] = new TH2F("hCompareRDiffvIntFluctSmallRPosRegion5", "Model-True vs Int Fluct Region 5, Pos Side, R Inside; int fluct charge inside (ions); shift difference (#mum)",nbinsint,-3e7,1e6,nbinsint,-10,4);
      hCompareRDiffvIntFluctLargeRPosRegion[1] = new TH2F("hCompareRDiffvIntFluctLargeRPosRegion1", "Model-True vs Int Fluct Region 1, Pos Side, R Outside; int fluct charge outside (ions); shift difference (#mum)",nbinsint,-3e7,1e6,nbinsint,-12,8);
